"""
RAG Stage-2 Hybrid Re-run — Gemma 4 31B on Qwen3 Top-K Hard Negatives
=====================================================================
...
Configs:
  RAG-D: Hybrid Qwen3 + BM25 (RRF) | 2 pos + 2 neg

Usage:
  CUDA_VISIBLE_DEVICES=0 python -u 6b.rag_stage2_hybrid_8192.py
  CUDA_VISIBLE_DEVICES=0 python -u 6b.rag_stage2_hybrid_8192.py --projects AAH BEAM
  CUDA_VISIBLE_DEVICES=0 python -u 6b.rag_stage2_hybrid_8192.py --force
...
RERUN_ROOT = "/home/jovyan/work/Thesis_Ijaaz/ground_truth_v3_clean_pipeline/RESULTS/RAG_STAGE2_HYBRID_V3_8192"
INDEXES_ROOT = os.path.join(RERUN_ROOT, "INDEXES")
STAGE1_MASTER_PATH = (
    "/home/jovyan/work/Thesis_Ijaaz/ground_truth_v3_clean_pipeline/"
    "RESULTS/RAG_STAGE1_V3_8192/MASTER_RAG_COMPARISON.json"
)
...
# ==================== STAGE-2 HYBRID CONFIG ====================
RAG_CONFIGS = [
    {"id": "RAG_D", "label": "RAG-D Hybrid 2+2", "retriever": "hybrid",
     "k_pos": 2, "k_neg": 2,
     "desc": "Hybrid (Qwen3 + BM25 via RRF), balanced contrastive."},
]


# Stage-1 baselines are embedded as a fallback so this script can always print
# the final A/B/C/D comparison even if the Stage-1 master file is elsewhere.
STAGE1_BASELINE_FALLBACK = {
    "RAG_A": {
        "label": "RAG-A MPNet 2+2",
        "macro_clean": {"precision": 0.5910, "recall": 0.8602, "f1": 0.6965,
                        "f2": 0.7846, "accuracy": 0.8062},
        "macro_conservative": {"precision": 0.5910, "recall": 0.8532, "f1": 0.6939,
                               "f2": 0.7796, "accuracy": 0.8052},
        "deployment_rollup": {
            "avg_time_per_pair_ms": 2047.4, "avg_retrieval_total_ms": 1.04,
            "avg_generation_ms": 2036.1, "avg_tokens_per_second_generated": 6.0,
            "macro_format_failure_rate_pct": 0.85, "macro_truncation_rate_pct": 0.88,
            "avg_vram_at_load_gb": 18.22, "avg_vram_peak_inference_gb": 21.49,
        },
        "total_failed_parses": 157, "n_projects": 8,
    },
    "RAG_B": {
        "label": "RAG-B Qwen3 2+2",
        "macro_clean": {"precision": 0.6202, "recall": 0.8491, "f1": 0.7135,
                        "f2": 0.7878, "accuracy": 0.8242},
        "macro_conservative": {"precision": 0.6202, "recall": 0.8382, "f1": 0.7093,
                               "f2": 0.7800, "accuracy": 0.8224},
        "deployment_rollup": {
            "avg_time_per_pair_ms": 2058.8, "avg_retrieval_total_ms": 2.96,
            "avg_generation_ms": 2044.8, "avg_tokens_per_second_generated": 6.0,
            "macro_format_failure_rate_pct": 1.21, "macro_truncation_rate_pct": 1.21,
            "avg_vram_at_load_gb": 18.22, "avg_vram_peak_inference_gb": 21.16,
        },
        "total_failed_parses": 206, "n_projects": 8,
    },
    "RAG_C": {
        "label": "RAG-C Qwen3 4+0",
        "macro_clean": {"precision": 0.5007, "recall": 0.9053, "f1": 0.6410,
                        "f2": 0.7751, "accuracy": 0.7416},
        "macro_conservative": {"precision": 0.5007, "recall": 0.8759, "f1": 0.6303,
                               "f2": 0.7546, "accuracy": 0.7381},
        "deployment_rollup": {
            "avg_time_per_pair_ms": 2073.8, "avg_retrieval_total_ms": 1.64,
            "avg_generation_ms": 2060.9, "avg_tokens_per_second_generated": 6.0,
            "macro_format_failure_rate_pct": 2.82, "macro_truncation_rate_pct": 2.85,
            "avg_vram_at_load_gb": 18.22, "avg_vram_peak_inference_gb": 21.18,
        },
        "total_failed_parses": 377, "n_projects": 8,
    },
}
...
def _load_stage1_comparison_configs():
    """Load Stage-1 A/B/C from disk when available, else use embedded fallback."""
    if os.path.exists(STAGE1_MASTER_PATH):
        try:
            with open(STAGE1_MASTER_PATH, "r", encoding="utf-8") as f:
                stage1 = json.load(f)
            configs = stage1.get("configs", {})
            if all(k in configs for k in ("RAG_A", "RAG_B", "RAG_C")):
                print(f"\nLoaded Stage-1 comparison from: {STAGE1_MASTER_PATH}")
                return {k: configs[k] for k in ("RAG_A", "RAG_B", "RAG_C")}, "stage1_master"
            print("\nStage-1 master found but missing A/B/C; using embedded fallback.")
        except Exception as exc:
            print(f"\nCould not read Stage-1 master ({exc}); using embedded fallback.")
    else:
        print(f"\nStage-1 master not found at {STAGE1_MASTER_PATH}; using embedded fallback.")
    return STAGE1_BASELINE_FALLBACK.copy(), "embedded_fallback"


def write_and_print_abcd_comparison(stage2_master):
    """Create final A/B/C/D comparison table after RAG-D finishes."""
    combined_configs, source = _load_stage1_comparison_configs()
    combined_configs.update(stage2_master.get("configs", {}))

    ordered = ["RAG_A", "RAG_B", "RAG_C", "RAG_D"]
    combined = {
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "model": BASE_MODEL,
        "test_set": "final_pairs_test (Qwen3 top-K hard negatives)",
        "stage1_source": source,
        "stage1_master_path": STAGE1_MASTER_PATH,
        "stage2_master_path": os.path.join(RERUN_ROOT, "MASTER_RAG_COMPARISON.json"),
        "configs": {k: combined_configs[k] for k in ordered if k in combined_configs},
    }

    out_path = os.path.join(RERUN_ROOT, "MASTER_RAG_ABCD_COMPARISON.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(combined, f, indent=2, default=str)

    print("\n\n" + "=" * 110)
    print("FINAL RAG A/B/C/D COMPARISON — accuracy")
    print("=" * 110)
    print(f"{'Config':<24} {'Metric':<14} {'P':>7} {'R':>7} {'F1':>7} {'F2':>7} {'Acc':>7} {'Fails':>7}")
    print("-" * 110)
    for cid in ordered:
        if cid not in combined["configs"]:
            continue
        c = combined["configs"][cid]
        clean = c.get("macro_clean") or {}
        cons = c.get("macro_conservative") or {}
        fails = c.get("total_failed_parses", 0)
        print(f"{c.get('label', cid):<24} {'clean':<14} "
              f"{clean.get('precision', 0):>7.4f} {clean.get('recall', 0):>7.4f} "
              f"{clean.get('f1', 0):>7.4f} {clean.get('f2', 0):>7.4f} "
              f"{clean.get('accuracy', 0):>7.4f} {fails:>7}")
        print(f"{'':<24} {'conservative':<14} "
              f"{cons.get('precision', 0):>7.4f} {cons.get('recall', 0):>7.4f} "
              f"{cons.get('f1', 0):>7.4f} {cons.get('f2', 0):>7.4f} "
              f"{cons.get('accuracy', 0):>7.4f}")
        print("-" * 110)
    ...
    print("\n" + "=" * 110)
    print("FINAL RAG A/B/C/D COMPARISON — deployment / RQ3 metrics")
    print("=" * 110)
    print(f"{'Config':<24} {'ms/pair':>9} {'retr_ms':>9} {'gen_ms':>9} "
          f"{'tok/s':>7} {'fail%':>6} {'trunc%':>7} {'load GB':>8} {'peak GB':>8}")
    print("-" * 110)
    for cid in ordered:
        if cid not in combined["configs"]:
        ...
        print(f"{c.get('label', cid):<24} "
              ...
              f"{(d.get('avg_vram_at_load_gb') or 0):>8.2f} "
              f"{(d.get('avg_vram_peak_inference_gb') or 0):>8.2f}")
    print(f"\nA/B/C/D comparison saved to: {out_path}")
    return combined
...
def main():
    ...
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print("=" * 110)
    print("RAG STAGE-2 HYBRID 8192 — GEMMA 4 31B (Qwen3 Top-K Hard Negatives)")
    print("=" * 110)
    ...
    if all_results:
        ...
        write_and_print_abcd_comparison(master)
